...
object SdkInstaller extends TaskBase {
  implicit val packageOrder
      : Ordering[com.android.repository.api.RemotePackage] =
    new Ordering[com.android.repository.api.RemotePackage] {
      override def compare(
          x: com.android.repository.api.RemotePackage,
          y: com.android.repository.api.RemotePackage
      ) = y.compareTo(x)
    }
  val platformOrder: Ordering[com.android.repository.api.RemotePackage] =
    new Ordering[com.android.repository.api.RemotePackage] {
      override def compare(x: RepoRemotePackage, y: RepoRemotePackage) =
        (x.getTypeDetails, y.getTypeDetails) match {
  ...
  def autoInstallPackage(
      ...
      showProgress: Boolean,
      slog: Logger,
      pred: Map[String, LocalPackage] => Boolean
  ): Option[RepoRemotePackage] = {
  ...
  def autoInstall(
      ...
      pkgfilter: Map[String, RepoRemotePackage] => Option[RepoRemotePackage]
  ): Option[RepoRemotePackage] = {
    val ind = SbtAndroidProgressIndicator(slog)
    val pkgs = retryWhileFailed("get local packages", slog)(
      sdkHandler.getSdkManager(ind).getPackages.getLocalPackages.asScala.toMap
  ...
  def platforms(
    ...
    val repomanager =
      sdkHandler.getSdkManager(PrintingProgressIndicator(showProgress))
    val downloader = SbtAndroidDownloader(sdkHandler.getFileOp)
    repomanager.loadSynchronously(
      1.day.toMillis,
      PrintingProgressIndicator(showProgress),
      downloader,
      null
    )
    val pkgs = repomanager.getPackages.getRemotePackages.asScala.toMap
      ...
      .toList
      .sorted(platformOrder)
      .map(_.getPath.stripPrefix("platforms;"))
    pkgs
  }
  ...
  def install(
      ...
      pkgfilter: Map[String, RepoRemotePackage] => Option[RepoRemotePackage]
  ): RepoRemotePackage = synchronized {
    val downloader = SbtAndroidDownloader(sdkHandler.getFileOp)
    val repomanager =
      sdkHandler.getSdkManager(PrintingProgressIndicator(showProgress))
    repomanager.loadSynchronously(
    ...
    remotepkg match {
      case None =>
        PluginFail(s"""No installable package found for $name
              |
              |Available packages:
              |${pkgs.keys.toList
                       ...
                       .map(pkgs)
                       .sorted(platformOrder)
                       .map("  " + _.getPath.substring(prefix.length))
                       .mkString("\n")}
           """.stripMargin)
      case Some(r) =>
        Option(r.getLicense).foreach { l =>
          ...
          println(
            s"By continuing to use sbt-android, you accept the terms of '$id'"
          )
        ...
        val installed =
          repomanager.getPackages.getLocalPackages.asScala.get(r.getPath)
        if (installed.forall(_.getVersion != r.getVersion)) {
          slog.info(s"Installing package '${r.getDisplayName}' ...")
          val installerF =
            SdkInstallerUtil.findBestInstallerFactory(r, sdkHandler)
          val installer = installerF.createInstaller(
            r,
            repomanager,
            downloader,
  ...
  def retryWhileFailed[A](err: String, log: Logger, delay: Int = 250)(
      f: => A
  ): A = {
    Iterator
      .continually(util.Try(f))
      .dropWhile { t =>
  ...
  def sdkPath(slog: sbt.Logger, props: java.util.Properties): String = {
    ...
    val path = (Option(System getenv "ANDROID_HOME") orElse
      ...
      slog.info(
        "ANDROID_HOME not set or invalid, using " +
    ...
    sys.props("com.android.tools.lint.bindir") =
      SdkLayout.tools(path).getCanonicalPath
    path
  }
  ...
  private def sdkpath(state: sbt.State): String =
    sdkPath(
      state.log,
      loadProperties(sbt.Project.extract(state).currentProject.base)
    )

  private[this] lazy val sdkMemo = scalaz.Memo
    .immutableHashMapMemo[File, (Boolean, Logger) => AndroidSdkHandler] { f =>
      val manager = AndroidSdkHandler.getInstance(f)
      if (!SdkLayout.repocfg.isFile) {
        println(
          s"Android SDK repository config ${SdkLayout.repocfg.getPath} does not exist, creating"
        )
  ...
  def sdkManager(
      ...
      showProgress: Boolean,
      slog: Logger
  ): AndroidSdkHandler = synchronized {
    sdkMemo(path)(showProgress, slog)
  }

  def installSdkAction: (sbt.State, Option[String]) => sbt.State =
    (state, toInstall) => {
      val log = state.log
      val ind = SbtAndroidProgressIndicator(log)
      val sdkHandler =
        sdkManager(sbt.file(sdkpath(state)), showProgress = true, log)
      val repomanager = sdkHandler.getSdkManager(ind)
      repomanager.loadSynchronously(
        1.day.toMillis,
        PrintingProgressIndicator(),
        SbtAndroidDownloader(sdkHandler.getFileOp),
        null
      )
      val newpkgs = repomanager.getPackages.getNewPkgs.asScala
        .filterNot(_.obsolete)
        .toList
        .sorted(platformOrder)
      toInstall match {
        case Some(p) =>
  ...
  def updateSdkAction
      : (sbt.State, Either[Option[String], String]) => sbt.State =
    (state, toUpdate) => {
      val log = state.log
      val ind = SbtAndroidProgressIndicator(log)
      val sdkHandler =
      ...
      val updates = repomanager.getPackages.getUpdatedPkgs.asScala
        .collect {
          case u if u.hasRemote => u.getRemote
        }
        .toList
        .sorted(platformOrder)
      ...
      toUpdate.left.foreach {
        case Some(_) =>
          updates.foreach { u =>
            install(sdkHandler, u.getDisplayName, "", showProgress = true, log)(
              _.get(u.getPath)
            )
          }
        case None =>
...
case class SbtAndroidDownloader(fop: FileOp) extends Downloader {
  ...
  override def downloadAndStream(
      url: URL,
      indicator: ProgressIndicator
  ): InputStream =
