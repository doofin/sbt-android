...
private[android] object parsers {
  ...
  def activityName(n: Node): String =
    n.attribute(Resources.ANDROID_NS, "name").head.text
  ...
  def activityParser: Initialize[State => Parser[Option[String]]] =
    loadForParser2(Keys.processManifest, Keys.applicationId) {
      (state, mfile, appid) =>
        val parser = for {
          f <- mfile if f.isFile
          pkg <- appid
          ...
          EOF.map(_ => None) | (Space ~> opt(
            (token(StringBasic.examples(pkg + "/")) ~ token(
              StringBasic.examples(names: _*)
            ))
              .map { case (a, b) => a + b }
          ))
        }
        parser getOrElse opt(
          Def.spaceDelimited("<activity name>").map(_.mkString(" "))
        )
    }
  ...
  private[this] def sdkManager(s: State): AndroidSdkHandler = {
    ...
    def existing = {
      val androids = e.structure.allProjects map (p =>
        sbt.ProjectRef(e.structure.root, p.id)
      ) filter { ref =>
        e.getOpt(Keys.projectLayout in ref).isDefined
      }
      androids.headOption.map(p => e.get(Keys.Internal.sdkManager in p))
    }

    e.getOpt(Keys.Internal.sdkManager)
      .orElse(existing)
      .getOrElse(
        SdkInstaller.sdkManager(
          sbt.file(
            SdkInstaller.sdkPath(s.log, Tasks.loadProperties(sbt.file(".")))
          ),
          showProgress = true,
          s.log
        )
  ...
  def updateSdkParser: State => Parser[Either[Option[String], String]] =
    state => {
      val ind = SbtAndroidProgressIndicator(state.log)
      val repomanager = sdkManager(state).getSdkManager(ind)
      val updates =
        repomanager.getPackages.getUpdatedPkgs.asScala.toList.collect {
  ...
  private[android] implicit val sjsonFileFormat: JsonFormat[File] =
    new JsonFormat[File] {
      override def write[J](obj: File, builder: Builder[J]): Unit =
        StringJsonFormat.write(obj.getCanonicalPath, builder)
      override def read[J](jsOpt: Option[J], unbuilder: Unbuilder[J]): File =
        sbt.file(StringJsonFormat.read(jsOpt, unbuilder))
    }
  def loadForParser2[P, T: JsonFormat, T2: JsonFormat](
      task: TaskKey[T],
      task2: TaskKey[T2]
  )(
      f: (State, Option[T], Option[T2]) => Parser[P]
  ): Initialize[State => Parser[P]] =
  ...
  def loadForParserI2[P, T: JsonFormat, T2: JsonFormat](
      ...
      init: Initialize[(State, Option[T], Option[T2]) => Parser[P]]
  ): Initialize[State => Parser[P]] =
    Def.setting {
      val ctx = sbt.Keys.resolvedScoped.value
      val f = init.value
      (s: State) =>
        f(s, loadFromContext(task, ctx, s), loadFromContext(task2, ctx, s))
    }
}
