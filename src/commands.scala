...
object Commands {
  ...
  val devicesAction: State => State = state => {
    ...
    if (list.isEmpty) {
      ...
      state.log.info(
        " %-22s %-16s %8s Android Version" format ("Serial", "Model", "Battery %")
      )
      // noinspection ScalaMalformedFormatString
      state.log.info(
        " %s %s %s ---------------" format ("-" * 22, "-" * 16, "-" * 9)
      )
      list foreach { dev =>
        val name = Option(dev.getAvdName) orElse
          Option(dev.getProperty(IDevice.PROP_DEVICE_MODEL)) getOrElse (if (
                                                                          dev.isEmulator
                                                                        ) "emulator"
                                                                        else
                                                                          "device")
        ...
        Try(
          state.log.info(
            f"$sel${dev.getSerialNumber}%-22s $name%-16s ${dev.getBattery.get}%8s%% ${dev
                .getProperty(IDevice.PROP_BUILD_VERSION)}%-6s API ${dev
  ...
  def localPathParser(entry: File): Parser[File] = {
    if (!entry.isDirectory) Parser.success(entry)
    else {
      val children = Option(entry.listFiles)
        .map { c =>
          val entries = c.map { e =>
      ...
      eof | token(StringBasic.examples(children.keys.toSeq.distinct: _*))
        .flatMap { e =>
          val entry2 = children.getOrElse(e, new File(entry, e.trim))
          val eof2 = EOF map { _ => entry2 }
          if (entry2.isFile) Parser.success(entry2)
          else {
            eof2 | (fileSep ~> Parser.opt(
              localPathParser(entry2)
            ) map (_ getOrElse entry2))
          }
        } | dot
    }
  }

  lazy val isWindows = System
    .getProperty("os.name", "")
  ...
  def androidPathParser(
      entry: FileEntry,
      fs: FileListingService
  ): Parser[(FileEntry, Option[String])] = {
    if (!entry.isDirectory) Parser.success((entry, None))
    else {
      ...
      eof | token(StringBasic.examples(children.keys.toSeq.distinct: _*))
        .flatMap { e =>
          val entries = e.split('/')
          val (entries2, remaining) =
            entries.toList.foldLeft((entry, List.empty[String])) {
              case ((f, ss), x) =>
          ...
          e2 match {
            case Some(entry2) =>
              ...
              if (!entry2.isDirectory)
                ...
                eof2 | (token("/") ~> Parser
                  .opt(androidPathParser(entry2, fs))
                  .map {
  ...
  def withFileService[A](
      s: State
  )(f: FileListingService => Parser[A]): Parser[A] =
    Try(targetDevice(sdkpath(s), s.log)).toOption.flatten.map(d =>
      f(d.getFileListingService)
    ) getOrElse
      Parser.failure("No devices connected")

  def androidFileParser(
      fs: FileListingService
  ): State => Parser[(FileEntry, Option[String])] = state => {
    token("/") ~> androidPathParser(fs.getRoot, fs)
  }
  val androidFileCommandParser: State => Parser[(FileEntry, Option[String])] =
    state => {
  ...
  private def printEntry(state: State, entry: FileEntry) {
    ...
    state.log.info(
      "%-8s %-8s %10s %s %s %s" format (entry.getOwner, entry.getGroup, entry.getSize,
  ...
  val adbPushParser: State => Parser[(File, (FileEntry, Option[String]))] =
    state =>
      Space ~> localFileParser(state) ~ withFileService(state) { fs =>
        EOF.map(_ => (fs.getRoot, None)) | (Space ~> androidFileParser(fs)(
          state
        ))
      }
  ...
  val adbPullParser: State => Parser[((FileEntry, Option[String]), File)] =
    state =>
      Space ~> withFileService(state) { fs =>
  ...
  val rebootParser: State => Parser[String] = state => {
    (EOF map (_ => null)) | (Space ~> Parser.oneOf(
      Seq("bootloader", "recovery")
    ))
  }
  ...
  private val wifiState = AttributeKey[String](
    "adb-wifi-device",
  ...
  val adbWifiAction: State => State = state => {
    ...
    if (adbWifiOn) {
      ...
      val ip = receiver.result.linesIterator
        .dropWhile(_.trim.split(" ")(0) != "inet")
        .toStream
        .headOption
        .fold("")(_.trim.split(" ")(1).takeWhile(_ != '/'))
      if (ip.nonEmpty) {
  ...
  def createProjectSbt(
    ...
    val plugin =
      s"""addSbtPlugin("org.scala-android" % "sbt-android" % "${BuildInfo.version}")""".stripMargin
    val version = Project.extract(state).get(sbt.Keys.sbtVersion)
    val useVersion = if (compareSbtVersion(version)) version else "1.2.8"
    if ((build * "*.sbt").get.isEmpty) {
      IO.writeLines(pluginSbt, plugin :: Nil)
    } else {
      state.log.warn(
        "SBT files already exist in project/, not creating project/android.sbt"
      )
    }
    ...
    val libs =
      "" ::
        ...
        Nil
    val plat =
      platform.toList.flatMap(p => "" :: s"""platformTarget := "$p"""" :: Nil)
  ...
  val createProjectSbtAction: (State, Option[String]) => State =
    (state, platform) => {
      createProjectSbt(multi = false, state, platform)
    }

  val createMultiProjectSbtAction: (State, Option[String]) => State =
    (state, platform) => {
      createProjectSbt(multi = true, state, platform)
    }

  val createProjectParser: State => Parser[Either[Unit, (String, String)]] =
    state => {
      ...
      val pkgName =
        token(Parser.oneOrMore(pkgPart) map (_ mkString ""), "<package>")
      val name = token(
        (idStart ~ Parser.zeroOrMore(id)) map { case (a, b) =>
          a + (b mkString "")
        },
        "<name>"
      )

      Parser.choiceParser(EOF, (Space ~> pkgName) ~ (Space ~> name))
    }
  val createMultiProjectAction
      : (State, Either[Unit, (String, String)]) => State = {
    case (state, maybe) =>
      maybe.left foreach { _ =>
        PluginFail("Usage: gen-multi-android <package-name> <name>")
      }
      (maybe.right map { case (pkg, name) =>
        createProject(multi = true, pkg, name, state.log)
        val sdkManager = SdkInstaller.sdkManager(
          file(sdkpath(state)),
          showProgress = true,
          state.log
        )
        val plat =
          SdkInstaller.platforms(sdkManager, showProgress = true).headOption
  ...
  val createProjectAction: (State, Either[Unit, (String, String)]) => State = {
    case (state, maybe) =>
      maybe.left foreach { _ =>
        PluginFail("Usage: gen-android <package-name> <name>")
      }
      (maybe.right map { case (pkg, name) =>
  ...
  private def createProject(
    ...
    val layout = ProjectLayout(base)
    if (layout.manifest.exists) {
      PluginFail("An android project already exists in this location")
    } else {
      ...
      val stringsTemplate = IO.readLinesURL(
        Resources.resourceUrl("android-project-template/strings.xml")
      ) mkString "\n"
      val manifestTemplate = IO.readLinesURL(
        Resources.resourceUrl("android-project-template/AndroidManifest.xml")
      ) mkString "\n"
      val sampleTemplate = IO.readLinesURL(
        Resources.resourceUrl("android-project-template/sample.scala")
      ) mkString "\n"
      val sampleJunit3Template = IO.readLinesURL(
        Resources.resourceUrl(
          "android-project-template/Junit3MainActivityTest.java"
        )
      ) mkString "\n"
      val sampleJunit4Template = IO.readLinesURL(
        Resources.resourceUrl(
          "android-project-template/Junit4MainActivityTest.java"
        )
      ) mkString "\n"
      ...
      IO.writeLines(
        base / "lint.xml",
        IO.readLinesURL(
          Resources.resourceUrl("android-project-template/lint.xml")
        )
      )
      IO.writeLines(
        base / "src/main/res/layout/main.xml",
        IO.readLinesURL(
          Resources.resourceUrl("android-project-template/main.xml")
        )
      )
      IO.writeLines(
        base / "src/main/res/anim/waving_arm.xml",
        IO.readLinesURL(
          Resources.resourceUrl("android-project-template/waving_arm.xml")
        )
      )
      IO.writeLines(
        base / "src/main/res/drawable/waving_scala_android.xml",
        IO.readLinesURL(
          Resources.resourceUrl(
            "android-project-template/waving_scala_android.xml"
          )
        )
      )
      IO.writeLines(
        base / "src/main/res/drawable/scala_android.xml",
        IO.readLinesURL(
      ...
      IO.write(
        base / "src/androidTest/java" / pkg
          .replace('.', '/') / "Junit3MainActivityTest.java",
        sampleJunit3Template.format(pkg, name)
      )
      IO.write(
        base / "src/androidTest/java" / pkg
          .replace('.', '/') / "Junit4MainActivityTest.java",
        sampleJunit4Template.format(pkg, name)
      )
      IO.write(
        base / "src/main" / "AndroidManifest.xml",
        manifestTemplate.format(pkg)
      )
      IO.write(
        base / "src/main/res/values/strings.xml",
        stringsTemplate.format(name)
      )
      IO.write(
        base / "src/main/scala" / pkg.replace('.', '/') / "MainActivity.scala",
  ...
  val projectParser: State => Parser[Option[ProjectRef]] = state => {
    ...
    import extracted._
    val prjs = structure.allProjects
      .map(rp => (rp.id, ProjectRef(structure.root, rp.id)))
      .filter { case (k, ref) =>
        getOpt(Keys.projectLayout in ref).isDefined
      }
      .toMap
    val ids =
      prjs.keys.filterNot(_ == currentRef.project).map(Parser.literal).toList

    if (ids.isEmpty) Parser.success(None)
    else if (prjs.size == 1) Parser.success(Some(prjs.values.head))
    else
      (Parser.success(None) <~ Parser.opt(NotSepClass | EOF)) | token(
        "/" ~> oneOf(ids)
      ).map(prjs.get)
  }

  val projectAndStringParser: State => Parser[(Option[ProjectRef], String)] =
    state => projectParser(state) ~ stringParser(state)

  val LOG_LINE: Regex = """^([A-Z])/(.+?)\( *(\d+)\): (.*?)$""".r
  val LOG_LINE_N: Regex = """^([A-Z])/(.+?)\( *(\d+): *(\d+)\): (.*?)$""".r
  def pidcatLogLine(d: IDevice, pkgOpt: Option[String], log: Logger)(
      pred: LogcatLine => Option[LogcatLine]
  ): String => Unit = {
    val PKG_PATTERN = """package:(\S+)""".r
    val PID_START =
      """^Start proc ([a-zA-Z0-9._:]+) for ([a-z]+ [^:]+): pid=(\d+) uid=(\d+) gids=(.*)$""".r
    val PID_START5_1 =
      """^Start proc (\d+):([a-zA-Z0-9._:]+)/[a-z0-9]+ for (.*)$""".r
    ...
    val uidSet: Set[String] = if (v >= 24) {
      ...
      val pkglist = sr1.result.split("\\s+").toList.map(_.trim).collect {
        case PKG_PATTERN(pkg) => pkg
      }
      pkglist.map { pkg =>
      ...
      if (l.trim.length > 0) {
        l.trim match {
          case LOG_LINE_N(level, tag, uid, pid, msg) =>
            if (uidSet(uid)) pred(LogcatLine(level, tag, pid, msg)) foreach {
              case LogcatLine(lvl, t, p, m) =>
                val colored =
          ...
          case LOG_LINE(level, tag, pid, msg) =>
            if (tag == "ActivityManager") {
              ...
              pred(LogcatLine(level, tag, pid, msg)) foreach {
                case LogcatLine(lvl, t, p, m) =>
                  val colored =
                    f"${colorLevel(lvl)} (${colorPid(p)}) ${colorTag(t)}%8s: $m"
                  scala.Console.out.println(colored)
  ...
  val pidcatGrepAction: (State, (Option[ProjectRef], String)) => State = {
    case (state, (prj, args)) =>
      ...
      targetDevice(sdk, state.log) map { d =>
        val receiver =
          new ShellLogging(pidcatLogLine(d, pkgOpt, state.log) { l =>
  ...
  def deviceApiLevel(dev: IDevice): Int =
    Option(dev.getProperty(IDevice.PROP_BUILD_API_LEVEL))
  ...
  val pidcatAction: (State, (Option[ProjectRef], String)) => State = {
    case (state, (prj, args)) =>
      ...
      targetDevice(sdk, state.log) map { d =>
        val receiver = new ShellLogging(
          pidcatLogLine(d, pkgOpt, state.log)(l =>
  ...
  val runasAction: (State, (Option[ProjectRef], String)) => State = {
    case (state, (prj, args)) =>
      ...
      targetDevice(sdk, state.log) map { d =>
        executeShellCommand(
          d,
          "run-as " + FileEntry.escape(packageName.get) + " " + args,
          state
        )
        state
      } getOrElse PluginFail("no device selected")
  ...
  def logcatLogLine(
      log: Logger
  )(pred: LogcatLine => Option[LogcatLine])(l: String): Unit = {
    if (l.trim.length > 0) {
      l.trim match {
        case LOG_LINE(lv, tg, p, m) =>
          pred(LogcatLine(lv, tg, p, m)) foreach {
  ...
  def highlightMatch(
      tagMatch: Iterator[Regex.Match],
      msgMatch: Iterator[Regex.Match],
      line: LogcatLine
  ): LogcatLine = {
    if (COLORS_ENABLED) {
  ...
  val logcatGrepAction: (State, String) => State = (state, args) => {
    val (regex, fpid) =
      args.split(" ").foldRight((List.empty[String], Option.empty[String])) {
        case (a, (as, pid)) =>
  ...
  val deviceParser: State => Parser[String] = state => {
    val names: Seq[Parser[String]] =
      deviceList(state) map (s => token(s.getSerialNumber))
  ...
  def wrapLine(text: String, width: Int): String = {
    ...
    val (lines, last, _) =
      text.split(" +").foldLeft((List.empty[String], "", width)) {
        case ((ls, l, rem), w) =>
  ...
  val showLicenseAction: (State, Option[String]) => State = (state, license) =>
    {
      license match {
        ...
        case None =>
          val licenses = Option(SdkLayout.sdkLicenses.listFiles)
            .getOrElse(Array.empty)
            .collect {
              case f if f.isFile =>
  ...
  private def sdkpath(state: State): String =
    SdkInstaller.sdkPath(
      state.log,
      Tasks.loadProperties(Project.extract(state).currentProject.base)
    )
  ...
  val variantParser: State => VariantParser = s => {
    ...
    projectParser(s) flatMap { r =>
      ...
      prj.fold(Parser.failure("No Android project selected"): VariantParser) {
        p =>
          val typeParser = extracted
            .getOpt(Keys.buildTypes in p)
            .fold {
              ...
              Parser
                .oneOf(literal("--") :: bt)
                .map(b => if ("--" == b) None else Option(b))
            }

          val flavorParser = extracted
            .getOpt(Keys.flavors in p)
            .fold {
              token("--").map(_ => Option.empty[String])
            } { flavors =>
              ...
              Parser
                .oneOf(literal("--") :: fl)
                .map(f => if ("--" == f) None else Option(f))
            }

          (Parser.success(p) ~ (EOF.map(_ => None) | token(
            Space ~> typeParser
          )) ~ (EOF.map(_ => None) | token(Space ~> flavorParser))).map {
            case (((a, b), c)) => (a, b, c)
          }
      }
    }
  ...
  private def adbRunas = Command(
    ...
    (
      "adb-runas",
      "execute shell commands on device as a debuggable package user"
    ),
  ...
  private def pidcatGrep = Command(
    "pidcat-grep",
    ("pidcat-grep", "grep through device logcat for a package"),
  ...
  private def genMultiAndroidSbt = Command(
    "gen-multi-android-sbt",
    (
      "gen-multi-android-sbt",
      "Create SBT files for existing multi-project android build"
    ),
  ...
  private def device = Command(
    "device",
    ("device", "Select a connected android device"),
