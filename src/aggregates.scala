...
object Aggregate {
  private[android] case class Retrolambda(
      enable: Boolean,
      classpath: Seq[File],
  ...
  private[android] case class Aapt(
      builder: Logger => AndroidBuilder,
      debug: Boolean,
  ...
  private[android] case class AndroidTest(
      debugIncludesTests: Boolean,
      classesJar: File,
  ...
  private[android] case class CollectResources(
      libraryProject: Boolean,
  ...
  private[android] case class Ndkbuild(
      javah: Seq[File],
      path: Option[String],
  ...
  private[android] case class Apkbuild(
      packagingOptions: PackagingOptions,
      apkbuildDebug: Boolean,
      debugSigningConfig: ApkSigningConfig,
      manifest: File,
  ...
  private[android] case class Manifest(
  ...
  private[android] case class Dex(
      inputs: (Boolean, Seq[File]),
      maxHeap: String,
      maxProcessCount: Int,
      multi: Boolean,
    ...
    def toDexOptions(incremental: Boolean = incremental): DexOptions =
      new DexOptions {
        ...
        override def getThreadCount: Integer =
        ...
        override def getAdditionalParameters: util.List[String] =
          additionalParams.asJava
      }
  }

  private[android] case class Proguard(
      useProguard: Boolean,
