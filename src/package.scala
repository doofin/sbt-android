...
package object android extends PluginFail {
      ...
      .settings(
        Seq(
          projectLayout := ProjectLayout(
            p.base.getCanonicalFile,
            Some(base.getCanonicalFile)
          ),
          sbt.Keys.target := base
        ) ++ settings: _*
      )
  }
  ...
  def withVariant(
    ...
    sbt.Keys.onLoad in Global := (sbt.Keys.onLoad in Global).value andThen {
      s =>
  ...
  private[android] def buildWith(
    ...
    projects flatMap { p =>
      Seq(
        ...
        compile in Compile := {
          compile in Compile dependsOn (packageT in Compile in p)
        }.value,
        localProjects +=
          LibraryProject(
            (projectLayout in p).value,
            (extraResDirectories in p).value,
            (extraAssetDirectories in p).value
          )((outputLayout in p).value),
        localProjects := {
          (localProjects.value ++ (localProjects in p).value).distinctLibs
        }
      )
    }
  }
  ...
  private[android] def onLoadOnce(
      key: AnyRef
  )(f: State => State): State => State = state => {
  ...
  private def extendVariant(
      key: sbt.SettingKey[Map[String, Seq[Setting[_]]]],
      name: String,
      ss: Seq[Setting[_]]
  ) =
    key := key { vs =>
  ...
  def buildType(name: String)(
      ss: Setting[_]*
  ): Def.Setting[Map[String, Seq[Setting[_]]]] =
    buildTypes += ((name, ss))

  def buildConfig(
      `type`: String,
      name: String,
      value: Def.Initialize[Task[String]]
  ): Def.Setting[Task[Seq[(String, String, String)]]] =
  ...
  ): Def.Setting[Option[ApkSigningConfig]] = {
  ...
  def apkDoNotStrip(name: String*): Def.Setting[PackagingOptions] =
    packagingOptions := {
      val opts = packagingOptions.value
      opts.copy(doNotStrip = opts.doNotStrip ++ name)
    }

  def apkExclude(name: String*): Def.Setting[PackagingOptions] =
    packagingOptions := {
      val opts = packagingOptions.value
      opts.copy(excludes = opts.excludes ++ name)
    }
  def apkPickFirst(name: String*): Def.Setting[PackagingOptions] =
    packagingOptions := {
      val opts = packagingOptions.value
      opts.copy(pickFirsts = opts.pickFirsts ++ name)
    }
  def apkMerge(name: String*): Def.Setting[PackagingOptions] =
    packagingOptions := {
      val opts = packagingOptions.value
      opts.copy(merges = opts.merges ++ name)
    }

  def manifestPlaceholder(
      key: String,
      value: String
  ): Def.Setting[Task[Map[String, String]]] =
  ...
  def apkVersionName(
      name: Def.Initialize[Task[String]]
