...
import sbt._
import sbt.Keys._
// import parsers.sjsonFileFormat // cause conflict implicits below:
/* ambiguous implicit values:
[error]  both value singleFileJsonFormatter in package sbt of type => sjsonnew.JsonFormat[sbt.package.File]
[error]  and value sjsonFileFormat in object parsers of type => sjsonnew.JsonFormat[java.io.File] */
import Resources.ANDROID_NS

/** @author
  *   pfnguyen
  */
trait AndroidProjectSettings extends AutoPlugin with ProjectExtra {
  ...
  private def allPluginSettings: Seq[Setting[_]] = inConfig(Compile)(
    Seq(
      dependencyClasspath := {
        Def.taskDyn {
          val dcp = dependencyClasspath.value
          if (
            debugIncludesTests.?.value.getOrElse(false) && apkbuildDebug.value()
          ) Def.task {
      ...
      packageConfiguration in packageBin := {
        ...
        implicit val output: Converter = o
        val sources = if (l) {
          c.sources filter { case (f, n) =>
            !f.getName.matches("R\\W+.*class")
          }
        } else {
          c.sources
        }
      ...
      javacOptions := {
        ...
        o ++ (if (!re) Seq("-bootclasspath", bcp)
              else Seq("-Xbootclasspath/a:" + bcp)) ++ debugOptions
      },
      javacOptions in doc := {
        (javacOptions in doc).value
          ...
          .foldRight(List.empty[String]) { (x, a) =>
    ...
    Classpaths.configSettings ++ Seq(
      ...
      pluginSettingsLoaded := {
        if (pluginSettingsLoaded.?.value.isDefined)
          fail(
            s"androidBuild has been applied to project ${thisProject.value.id} more than once"
          )
        true
      },
      lint := {
      ...
      dependencyClasspath :=
        Def.taskDyn {
          ...
          if (
            apkbuildDebug.value() && debugIncludesTests.?.value.getOrElse(false)
          ) Def.task {
            val s = streams.value
            val tcp = (externalDependencyClasspath in AndroidTest).value
            cp foreach { a =>
              s.log.debug(
                s"${a.data.getName} => ${a.get(configuration.key)}: ${a.get(moduleID.key)}"
              )
            }
            val newcp = cp ++ tcp
      ...
      antLayoutDetector := {
        ...
        projectLayout.value match {
          case a: ProjectLayout.Ant if a.manifest.exists =>
            ...
            log.warn(
              "  this format has been deprecated in favor of modern layouts"
            )
            log.warn(
              "  If this is what you want, set 'antLayoutDetector in Android := ()'"
            )
          case _ =>
        }
      },
      ...
      apklibArtifact := normalizedName { n =>
        Artifact(n, "apklib", "apklib")
      }.value,
      ...
      buildConfigOptions := {
        ...
        List(
          ...
          versionName.value.toList.map(n =>
            ("String", "VERSION_NAME", s""""$n"""")
          ) ++
      ...
      rsSupportMode := properties { p =>
        Try(p.getProperty("renderscript.support.mode").toBoolean)
          .getOrElse(false)
      }.value,
      rsOptimLevel := 3,
...
[error]  and value sjsonFileFormat in object parsers of type => sjsonnew.JsonFormat[java.io.File] */
      ...
      versionCode := manifest.value
        .attribute(ANDROID_NS, "versionCode")
        .map(_.head.text.toInt),
      versionName := manifest.value
        .attribute(ANDROID_NS, "versionName")
        .map(_.head.text) orElse Some(version.value),
      packageForR := manifest
        .map(m => m.attribute("package").get.head.text)
        .value,
      applicationId := packageForR.value,
      targetSdkVersion := {
        val m = manifest.value
        val usesSdk = m \ "uses-sdk"
        val v = String.valueOf(platformApi.value)
        if (usesSdk.isEmpty) v
        else
          usesSdk.head.attribute(ANDROID_NS, "targetSdkVersion").fold(v) {
            _.head.text
          }
      },
      minSdkVersion := {
        ...
        val usesSdk = m \ "uses-sdk"
        if (usesSdk.isEmpty) usemin
        else
          usesSdk.head.attribute(ANDROID_NS, "minSdkVersion").fold(usemin) {
            _.head.text
          }
      },
      ...
      ndkPath := {
        ...
        def propertiesSetting =
          Option(props getProperty "ndk.dir").map("'ndk.dir' property" -> _)
        def envVarSetting = Option(System getenv "ANDROID_NDK_HOME").map(
          "'ANDROID_NDK_HOME' env var" -> _
        )
        def sdkBundleFallback = Some(SdkLayout.ndkBundle(spath))
          .filter(_.isDirectory)
          .map("ndk-bundle" -> _.absolutePath)

        val alternatives =
          propertiesSetting ++ envVarSetting ++ sdkBundleFallback
        val foundNdk = alternatives.view
          .map {
      ...
      builder := {
        ...
        bldr.setLibraryRequests(reqs.map { case ((nm, required)) =>
          new LibraryRequest(nm, required)
        }.asJava)
      ...
      bootClasspath := builder
        .value(sLog.value)
        .getBootClasspath(false)
        .asScala map Attributed.blank,
      sdkManager := SdkInstaller
        .sdkManager(file(sdkPath.value), showSdkProgress.value, sLog.value),
      buildToolInfo := {
        val slog = sLog.value
        ...
        val r = buildToolsVersion.value map { version =>
          val bti =
            SdkInstaller.retryWhileFailed("fetch build tool info", slog) {
          ...
          val tools =
            SdkInstaller.retryWhileFailed("determine latest build tools", slog)(
          ...
          SdkInstaller
            .autoInstall(
              ...
              buildTools
                .sorted(SdkInstaller.packageOrder)
                .dropWhile(_.getVersion.getPreview > 0)
                .headOption
            }
            .fold {
        ...
        if (r == null)
          PluginFail(
            "Android SDK installation registry failed to update, restart SBT"
          )
        r
      },
      platformTarget := {
        ...
        Option(p.getProperty("target")) orElse {
          sLog.value.warn(
            "`platformTarget` not set, automatically detecting latest..."
          )
          val plat = SdkInstaller
            .platforms(sdkManager.value, showSdkProgress.value)
            .headOption
          plat.foreach { t =>
      ...
      platform := {
        ...
        val slog = sLog.value
        val sdkHandler = sdkManager.value
        AndroidGlobalPlugin.platformTarget(
          targetHash,
          sdkHandler,
          showSdkProgress.value,
          slog
        )
        val logger = ilogger.value(slog)
        sdkLoader.value.getTargetInfo(
          targetHash,
          buildToolInfo.value.getRevision,
          logger,
      ...
      m2repoCheck := {
        ...
        val (needSupp, needGms) = libs.foldLeft((false, false)) {
          case ((supp, gms), mid) =>
            (
              supp || supportOrgs(mid.organization),
              gms || gmsOrgs(mid.organization)
            )
        }
        if (needSupp || needGms) {
    ...
    watchSources ++= Def.task {
      ...
      (layout.testSources +: layout.jni +: layout.res +: extras) flatMap {
        path =>
          (path ** filter).get
      }
    }.value
