...
object VariantSettings {
  ...
  private[android] val onUnloadSkip =
    AttributeKey[Boolean]("on-unload-skip", "skip onUnload if set")
  private[android] val originalSettings = AttributeKey[Seq[Setting[_]]](
    "original-settings",
  ...
  val variantSettings = AttributeKey[VariantSettings](
    "flavor-settings",
    "Tracks current build, project, and setting flavor modifications.",
    KeyRanks.DSetting
  )

  def variant(state: State): VariantSettings =
    state.get(variantSettings) getOrElse empty(state)
  ...
  def clearVariant(s: State): State = {
    ...
    withVariant(s)(variants =>
      reapply(
        session,
        variants.copy(
          append = variants.append - session.current,
          status = variants.status - session.current
        ),
        Project.structure(s),
        s
      )
  ...
  def clearAllVariants(s: State): State = {
    ...
    withVariant(s)(variants =>
      reapply(session, variants.clearExtraSettings, Project.structure(s), s)
    )
  }
  def clearVariant(s: State, ref: ProjectRef): State = {
    ...
    withVariant(s)(variants =>
      reapply(
        session,
        variants
          .copy(append = variants.append - ref, status = variants.status - ref),
        Project.structure(s),
        s
      )
    )
  ...
  def fixProjectScope(prj: ProjectRef): Setting[_] => Setting[_] = s => {
    val mapper = new Def.MapScoped {
      override def apply[T](a: Def.ScopedKey[T]) = {
        val scope0 =
          if (a.scope.project == This)
            a.scope.copy(project = Select(prj))
          else a.scope
        val scope1 =
          if (scope0.task == This) scope0.copy(task = Zero) else scope0
        val scope2 =
          if (scope1.extra == This) scope1.copy(extra = Zero) else scope1
        val scope3 =
          if (scope2.config == This) scope2.copy(config = Zero) else scope2
        a.copy(scope = scope3)
      }
    }
  ...
  def setVariant(
    ...
    if (buildType.nonEmpty || flavor.nonEmpty) {
      ...
      val ss3 = variantOptions(buildType, project, s) ++ variantOptions(
        flavor,
        project,
        s
      ) ++ variantOptions(
      ...
      val newVariant = variants.copy(
        append = variants.append + ((project, ss2)),
        status = variants.status + ((project, (buildType, flavor)))
      )
      val bt = buildType.getOrElse("(none)")
      val fl = flavor.getOrElse("(none)")
      s.log.info(
        s"Applying variant settings buildType=$bt flavor=$fl to ${project.project}..."
      )
  ...
  def variantOptions(
    ...
    def overlayManifest(v: Option[String]): Seq[Setting[_]] =
      v.fold(Seq.empty[Setting[_]]) { t =>
  ...
  def showVariantStatus(s: State, project: ProjectRef): State = withVariant(s) {
    variants =>
      val extracted = Project.extract(s)
      ...
      val abt =
        if (buildTypes.isEmpty) "  (none)"
        else buildTypes.keys map ("  " + _) mkString "\n"
      val af =
        if (flavors.isEmpty) "  (none)"
  ...
  def append(s: State, ss: Seq[Setting[_]]): State = {
    ...
    val appendSettings = Load.transformSettings(
      Load.projectScope(e.currentRef),
      e.currentRef.build,
      e.rootProject,
      ss
    )
    ...
    val newStructure =
      Load.reapply(newSettings, e.structure)(Project.showContextKey2(e.session))
    Project.setProject(
      e.session.copy(original = newSettings),
      newStructure,
      s.put(
        originalSettings,
        s.get(originalSettings).getOrElse(e.session.original) ++ appendSettings
      ).put(onUnloadSkip, true)
    )
  }

  def reapply(
      session: SessionSettings,
      newVariant: VariantSettings,
    ...
    val loggerInject = sbt.Keys.sLog in GlobalScope := new Logger {
      ...
      private[this] def slog: Logger = Option(ref.get) getOrElse sys.error(
    ...
    val session2 =
      session.copy(original = originals ++ withLogger.mergeSettings)
    val newStructure = Load.reapply(session2.mergeSettings, structure)(
      showContextKey(session2, structure)
    )
  ...
  private[this] def hasExplicitGlobalLogLevels(s: State): Boolean =
    s.get(explicitGlobalLogLevels) getOrElse false
...
final case class VariantSettings(
    append: VariantMap,
    rawAppend: Seq[Setting[_]],
    status: VariantStatus
) {
  def appendRaw(ss: Seq[Setting[_]]): VariantSettings =
    copy(rawAppend = rawAppend ++ ss)
  def mergeSettings: Seq[Setting[_]] = merge(append) ++ rawAppend
  def clearExtraSettings: VariantSettings =
    VariantSettings(Map.empty, Nil, Map.empty)

  private[this] def merge(map: VariantMap): Seq[Setting[_]] =
    map.values.toList.flatten[Setting[_]]
}
