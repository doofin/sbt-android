...
/** @author
  *   pfnguyen
  */
trait AndroidAppSettingsPlugin extends AutoPlugin {
  override def projectSettings = inConfig(Android)(
    List(
      ...
      zipalignPath := {
        ...
        if (pathInBt.exists)
          ...
          if (!zipalign.exists)
            fail(
              "zipalign not found at either %s or %s" format (pathInBt, zipalign)
            )
    ...
    libraryDependencies += Def
      .setting(
        "net.sf.proguard" % "proguard-base" % proguardVersion.value % AndroidInternal.name
      )
      .value,
    managedClasspath in AndroidInternal := Classpaths.managedJars(
      AndroidInternal,
  ...
  private[this] val stableProguardConfig = Def.taskDyn {
    ...
    if (
      rulecheck.nonEmpty && checkfiles != rulecheck && useProguardInDebug.value
    ) Def.task {
