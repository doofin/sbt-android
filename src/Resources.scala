...
object Resources {
  ...
  def doCollectResources(
    ...
    (
      depassets ++
        libs.collect { case Dependencies.LibraryProject(_, _, as) =>
          as.filter(_.isDirectory)
        }.flatten ++
        libs.collect {
    ...
    val res =
      extraRes.map(pkg -> _) ++ Seq(layout.res, rsResources).map(pkg -> _) ++
        libs.collect { case l @ Dependencies.LibraryProject(_, rs, _) =>
          rs.filter(_.isDirectory).map(l.pkg -> _)
        }.flatten ++
        libs.flatMap { l =>
          (if (l.layout.res.isDirectory) List(l.layout.res) else Nil) ++
            (if (l.layout.rsRes.isDirectory) List(l.layout.rsRes)
             else Nil) map (l.pkg -> _)
        }
    ...
    val depres = collectdeps(libs) collect {
      ...
      case n if n.getResFolder.isDirectory => n.pkg -> n.getResFolder
    }
    ...
    val respaths = depres ++ res.reverse ++
      ((if (layout.res.isDirectory) Seq(layout.res) else Seq.empty) ++
        (if (noTestApk && layout.testRes.isDirectory)
           Seq(layout.res)
         else Seq.empty)).map(pkg -> _)
    val vectorprocessor = new VectorDrawableRenderer(
      if (renderVectors) minSdk else math.max(minSdk, 21),
      layout.generatedVectors,
      Set(Density.MEDIUM, Density.HIGH, Density.XHIGH, Density.XXHIGH).asJava,
      logger
    )
    ...
    FileFunction.cached(cache / "nuke-res-if-changed", FilesInfo.lastModified) {
      in =>
  ...
  def incrResourceMerge(
    ...
    if (!merger.loadFromBlob(blobDir, true)) {
      ...
      val exists = changes.added ++ changes.removed ++ changes.modified exists {
        file =>
          val status =
            if (changes.added contains file)
              FileStatus.NEW
          ...
          if (vstatus == FileValidity.FileStatus.UNKNOWN_FILE) {
            ...
            val targetFile =
              resTarget / (file relativeTo fileValidity.getSourceFile).get.getPath
            ...
            try {
              if (
                !fileValidity.getDataSet.updateWith(
              ...
              case e: Exception =>
                slog.warn(
                  s"Unable to handle changed file: $file (${e.getMessage})"
                )
                slog.trace(e)
                merge()
                true
  ...
  def makeAapt(
    ...
    val filteredLogger = new ILogger {
      ...
      override def verbose(msgFormat: String, args: AnyRef*) =
        squelch(logger.debug(_: String), msgFormat, args)
      override def warning(msgFormat: String, args: AnyRef*) =
        squelch(logger.warn(_: String), msgFormat, args)
      override def error(t: Throwable, msgFormat: String, args: AnyRef*) =
        squelch(logger.error(_: String), msgFormat, args)
      override def info(msgFormat: String, args: AnyRef*) =
        squelch(logger.info(_: String), msgFormat, args)
    }
  ...
  def aapt(
    ...
    val aapt = makeAapt(
      bldr,
      bldr.getTargetInfo.getBuildTools,
      aaptTemp,
      false,
      false,
      logger
    ...
    aaptConfig.setVariantType(
      if (lib) VariantType.LIBRARY else VariantType.DEFAULT
    )
  ...
  lazy val androidJarMemo = scalaz.Memo.immutableHashMapMemo[File, ClassLoader](
    ClasspathUtilities.toLoader(_: File)
  )
  lazy val androidClassMemo =
    scalaz.Memo.immutableHashMapMemo[(String, String), Option[String]] {
      case ((j, cls)) =>
        Try(androidJarMemo(file(j)).loadClass(cls).getName).toOption
    }
  ...
  def generateTR(
      ...
      sv: String,
      l: Seq[LibraryDependency],
      f: Boolean,
      ids: Boolean,
      includeAar: Boolean,
      withViewHolders: Boolean,
      i: Seq[String],
      s: TaskStreams
  ): Seq[File] = {
    ...
    if (!t)
      ...
      FileFunction
        .cached(
          s.cacheDirectory / "typed-resources-generator",
          FilesInfo.hash
        ) { in =>
          if (in.nonEmpty) {
            ...
            def warn(res: Seq[(String, String)]) = {
              ...
              val overrides =
                res.groupBy(r => r._1) filter (_._2.toSet.size > 1) collect {
                  case (k, v) =>
                    s.log.warn(
                      "%s was reassigned: %s" format (k,
                      v map (_._2) mkString " => ")
                    )
            ...
            val layoutTypes = warn(for {
              ...
              l <- classForLabel(j, layout.label)
                .orElse(Some("android.view.View"))
            ...
            val trTemplate =
              IO.readLinesURL(resourceUrl("tr.scala.template")) mkString "\n"
            ...
            val combined =
              reduceResourceMap(Seq(rms1, rms2, rms3)).filter(_._2.nonEmpty)

            val combined1 = combined.map { case (k, xs) =>
              val k2 =
                if (k endsWith "-array") "array"
                else if (k == "menu_item") "id"
                else k
              val trt = trTypes(k)
            ...
            val combined2 = combined1.foldLeft(emptyResourceMap) {
              case (acc, (k, xs)) =>
                val k2 = if (k endsWith "-array") "array" else k
                acc + ((k2, acc(k2) ++ xs))
            }

            val trs = combined2.foldLeft(List.empty[String]) {
              case (acc, (k, xs)) =>
                val k2 = if (k endsWith "-array") "array" else k
                s"""
               |  object $k2 {
            ...
            IO.write(
              ...
              if (ids) {
                resources map { case (k, v) =>
                  "  final val %s = TypedResource[%s](R.id.%s)" format (wrap(
                    k
                  ), v, wrap(k))
                } mkString "\n"
              } else
                "  // TypedResource ID generation disabled by 'typedResourcesIds := false'",
              layoutTypes map { case (k, v) =>
                "    final val %s = TypedLayout[%s](R.layout.%s)" format (wrap(
                  k
                ), v, wrap(k))
    ...
    val rms = valuesxmls.map { xml =>
      ...
      val itemEntries = items.flatMap { node =>
        (for {
          ...
          typ <- attributeText(node, "type")
            .filter(itemTypes)
            .orElse(attributeText(node, "format").flatMap(formatTypes.get))
      ...
      def foldKey(key: String): (ResourceMap, scala.xml.Node) => ResourceMap =
        (m, node) => {
          node
            .attribute("name")
            .flatMap(_.headOption)
            .fold(m)(n => m + ((key, n.text :: m(key))))
        }
  ...
  def processResourceTypeDirs(
    ...
    val rms2 = for {
      ...
      ((res / restype * "*").get ++ (res * s"$restype-*" * "*").get)
        .map(_.getName.takeWhile(_ != '.'))
        .toList
        .filter(_.nonEmpty)
    rms2.foldLeft(emptyResourceMap) { case (m, (t, xs)) =>
      m + (t -> (m(t) ++ xs))
    }
  }

  def processMenuItems(
      resdirs: Seq[File],
      re: Regex,
      s: TaskStreams
  ): ResourceMap = {
  ...
  def generateViewHolders(
    ...
    if (!generate) Nil
      ...
      case class LayoutInclude(
          name: Option[String],
          id: Option[String],
          layout: String
      ) extends LayoutEntry
      case class LayoutView(name: String, id: String, viewType: String)
          extends LayoutEntry
      case class LayoutStructure(
          name: String,
          rootView: String,
      ...
      def parseLayout(
        ...
        val (r, c) = xml.descendant_or_self.foldLeft(
          (Option.empty[(Option[String], String)], List.empty[LayoutEntry])
        ) { case ((root, children), n) =>
          val viewId = n.attribute(ANDROID_NS, "id") map {
            _.head.text
          } flatMap idNameFromString
          if (n.label == "layout" || n.label == "#PCDATA") // noop, skip
            (root, children)
          else if (n.label == "fragment")
            (root, children)
          else if (n.label == "include") {
            val includeId = n.attribute(
              ANDROID_NS,
              "id"
            ) map (_.head.text) flatMap idNameFromString
...
//              n.label
            (
              root,
              LayoutView(viewId.get._1, viewId.get._2, n.label) :: children
            )
          }
        }
      ...
      FileFunction
        .cached(
          s.cacheDirectory / "viewHoldersGenerator",
          FilesInfo.lastModified
        ) { in =>
          val vhs = pkg.split("\\.").foldLeft(layout.gen) {
            _ / _
          } / "viewHolders.scala"
          val vhTemplate = IO.readLinesURL(
            resourceUrl("viewHolders.scala.template")
          ) mkString "\n"
          vhs.delete()
          ...
          val layouts = files.map { f =>
            ...
            LayoutFile(
              f.getName.stripSuffix(".xml"),
              parent.split("-").toList.drop(1).sorted,
              f
            )
          }
          val grouped = layouts.groupBy(_.name).mapValues(_.sorted)
          val viewholders = grouped
            .map { case (n, data) =>
              val main = data.head
              val rest = data
                .drop(1)
                .filter(
                  _.configs.nonEmpty
                ) // handle the case of duplicate file names in dependent projects
              ...
              struct.copy(configs =
                rest
                  .map(l =>
                    parseLayout(
                      l.configs.map(_.capitalize).mkString,
                      l.path,
                      l.configs.toSet,
                      false
                    )
          ...
          def findClosestConfig(
              required: Set[String],
              items: List[LayoutStructure]
          ): LayoutStructure =
            // included head already, skip
            items
              .drop(1)
              .foldLeft((0, items.head)) { case ((count, best), i) =>
          ...
          def processViews(structure: LayoutStructure, _seen: Set[String])
              : (Set[String], List[String]) = {
            def generateFindViewById(
                id: String,
                castType: String,
                platformApi: Int
            ): String = {
              if (platformApi >= 26) {
                ...
                val cast =
                  if (castType == "android.view.View") ""
                  else s".asInstanceOf[$castType]"
                s"rootView.findViewById($id)$cast"
              }
            }

            structure.views.foldLeft((_seen, List.empty[String])) {
              case ((seen, items), e) =>
                e match {
                  case LayoutView(name, id, viewType) =>
                    val castType =
                      classForLabel(j, viewType).getOrElse("android.view.View")
                    val actualName = takeAlternative(seen, name)
                    if (seen(name)) {
                      s.log.warn(
                        s"TVH: '$name' already used in '${structure.name}', using '$actualName'"
                      )
                    }

                    val typedMember =
                      s"lazy val ${wrap(actualName)}: $castType = ${generateFindViewById(id, castType, platformApi)}"
                  ...
                  case LayoutInclude(name, id, included) =>
                    if (!viewholders.contains(included)) {
                     ...
                     |or disable TypedViewHolders `typedViewHolders := false`""".stripMargin
                      )
                    }
                    ...
                    if (seen(ident))
                      s.log.warn(
                        s"TVH: '$ident' already used in '${structure.name}', using '$actualIdent'"
                      )
                    val wrapi = wrap(included)
                    if (vh.rootView == "merge") {
                      (
                        seen + actualIdent,
                        s"    lazy val $wrapId = TypedViewHolder.${wrapi}(rootView)" :: items
                      )
                    } else {
                      id.orElse(vh.rootId)
                        .fold {
                          val (newseen, newviews) = processViews(
                            findClosestConfig(
                              structure.config,
                              vh :: vh.configs
                            ),
                          ...
                          val castType = classForLabel(j, vh.rootView)
                            .getOrElse("android.view.View")
          ...
          val (vhlist, facts) = viewholders.values
            .foldLeft(
              (
                Set("setContentView", "inflate", "from"),
              ...
              if (seen(struct.name))
                s.log.warn(
                  s"TVH: '${struct.name}' already used in 'TypedViewHolder', using '$actualName'"
                )
              val wname = wrap(actualName)
              val rootClass =
                classForLabel(j, struct.rootView).getOrElse("android.view.View")
              val (_, views) = processViews(
                struct,
                Set("rootView", "rootViewId") ++ struct.rootId
                  .map(_.split('.').last)
              )
              val configs = struct.configs map { cfg =>
                val (_, cfgviews) = processViews(
                  cfg,
                  Set("rootView", "rootViewId") ++ struct.rootId
                    .map(_.split('.').last)
                )
              ...
              val vh =
                s"""  final case class $wname(rootView: $rootClass) extends TypedViewHolder[$rootClass] {
                      |    val rootViewId = ${struct.rootId.getOrElse("-1")}
                      |    rootView.setTag(R.layout.${wrap(
                    struct.name
                  )}, this)$rootName
                      |${views.mkString("\n")}
                      |${configs.mkString("\n")}
              ...
              val f =
                s"""  implicit val ${actualName}_ViewHolderFactory: TypedViewHolderFactory[TR.layout.${wrap(
                    struct.name
                  )}.type] { type VH = $vhname } = new TypedViewHolderFactory[TR.layout.${wrap(
                    struct.name
                  )}.type] {
                      |    type V = $rootClass
                      |    type VH = $vhname
          ...
          IO.write(
            vhs,
            vhTemplate format (pkg, facts.mkString("\n"), vhlist
              .mkString("\n")) replace ("\r", "")
          )
          Set(vhs)
        }(files.toSet)
