...
object Tasks extends TaskBase {
  ...
  val buildConfigGeneratorTaskDef = Def.task {
    ...
    o.groupBy(_._2).values.map(_.head) foreach { case (tpe, n, value) =>
      b.addField(tpe, n, value)
    }
    b.generate()
    l collect {
      case a: ApkLibrary         => a
  ...
  def moduleForFile(u: UpdateReport, f: File): ModuleID = {
    ...
    val map: Map[File, ModuleID] = {
      ...
      fm.foldLeft(Map.empty[File, ModuleID]) { case (m, (k, mid)) =>
        ...
        m.updated(
          k,
          mid2.withConfigurations(
            concat(mid.configurations, mid2.configurations)
          )
        )
      }
    }
  ...
  val aarsTaskDef = Def.task {
    ...
    def excludeTests(m: ModuleID): Boolean = {
      val isTest = m.configurations.exists(c =>
        c.split(",").forall(s => s.startsWith("test") || s == "androidTest")
      )
    ...
    val subaars = ta
      .collect { case a: AarLibrary => a }
      .map { a => moduleString(a.moduleID) }
      .toSet
    ...
    (libs flatMap { l =>
      ...
      if (excludeTests(m)) {
        ...
        if (!subaars(moduleString(m))) {
          ...
          s.log.debug(
            m + " was already included by a dependent project, skipping"
          )
          None
        }
      } else {
      ...
      unpackAar(
        a,
        dest / ("localAAR-" + a.getName),
        "localAAR" % a.getName % "LOCAL",
        s.log
      )
    })
  }
  ...
  val androidTestAarsTaskDef = Def.task {
    ...
    def testOnly(m: ModuleID): Boolean = {
      !withTest && m.configurations.exists(c =>
        c.split(",").forall(_ == "androidTest")
      )
    }
    val libs = u.matching(artifactFilter(`type` = "aar"))
    libs.flatMap { l =>
      ...
      if (testOnly(m)) {
        ...
        if (testAarWarning.value)
          s.log.warn(
            "androidTest: test-only aar will not have resources merged " + m
          )
...
//        logger, file("/"), s)
      ...
      def copyDirectory(src: File, dst: File) {
        IO.copy(
          (src.allPaths --- (src ** "R.txt")) pair Path.rebase(src, dst),
          overwrite = false,
  ...
  val resolvedAars = Def.task {
    (update in Compile).value
      .configuration(ConfigRef("compile"))
      .map(_.details)
      .getOrElse(Nil)
  ...
  val checkAarsTaskDef = Def.task {
    ...
    transitiveAars.value
      .collect { case a: AarLibrary ⇒
        a → resolved.find(_.name == a.moduleID.name)
      }
      .collect { case (a, Some(r)) if r.latest > a.moduleID.revision ⇒ r }
      .foreach(reportIncompatibleAars)
  }

  def reportIncompatibleAars(aar: OrganizationArtifactReport)(implicit
      log: Logger,
  ...
  val typedResourcesGeneratorTaskDef = Def.task {
    Resources.generateTR(
      typedResources.value,
      rGenerator.value ++ (collectResources.value._2 ** "*.xml").get,
      packageForR.value,
      projectLayout.value,
      platformApi.value,
      platformJars.value,
      (scalaVersion in ThisProject).value,
      libraryProjects.value,
      typedResourcesFull.value,
      typedResourcesIds.value,
  ...
  def ndkbuild(
      manager: AndroidSdkHandler,
      layout: ProjectLayout,
      args: Seq[String],
      envs: Seq[(String, String)],
      ndkHome: Option[String],
      srcs: File,
    ...
    if (hasJni) {
      val ndk = ndkHome.getOrElse {
        val bundlePath =
          SdkLayout.ndkBundle(manager.getLocation.getAbsolutePath)
      ...
      FileFunction.cached(layout.ndkObj, FilesInfo.lastModified) { _ =>
        val env = Seq(
          "NDK_PROJECT_PATH" -> (layout.jni / "..").getAbsolutePath,
        ...
        val ndkbuildFile =
          if (!Commands.isWindows) file(ndk) / "ndk-build"
          else {
  ...
  val ndkJavahTaskDef = Def.task {
    ...
    if (natives.nonEmpty) {
      val javah = Seq(
        ...
        "-bootclasspath",
        boot.map(_.data) mkString File.pathSeparator
      ) ++ natives
  ...
  val collectJniTaskDef = Def.task {
    ...
    def libJni(lib: LibraryDependency): Seq[File] =
      Seq(lib.getJniFolder, lib.layout.rsLib).filter(_.isDirectory) ++
        lib.getLibraryDependencies.asScala.flatMap(l =>
          libJni(l.asInstanceOf[LibraryDependency])
        )
  ...
  val collectResourcesTaskDef = Def.task {
    ...
    val assets = layout.assets +: ea
      .map(_.getCanonicalFile)
      .distinct
      .flatMap(a => (a ** FileOnlyFilter).get)
    withCachedRes(
      s,
      "collect-resources-task",
      assets ++ normalres(layout, er, libs),
      genres(layout, libs)
    ) {
      val res = Resources.doCollectResources(
        bldr(s.log),
        cra.packageForR,
        minLevel,
        noTestApk,
  ...
  val packageApklibMappings = Def.task {
    ...
    (PathFinder(layout.processedManifest) pair Path.flat) ++
      (PathFinder(javaSource) ** "*.java" pair Path.rebase(
        javaSource,
        "src"
      )) ++
      (PathFinder(scalaSource) ** "*.scala" pair Path.rebase(
        scalaSource,
        "src"
      )) ++
  ...
  val packageResourcesTaskDef = Def.task {
    ...
    withCachedRes(
      s,
      p.getName,
      layout.manifest +: normalres(layout, extrares, libs),
      genres(layout, libs)
    ) {
  ...
  def genres(layout: ProjectLayout, libs: Seq[LibraryDependency])(implicit
      out: BuildOutput.Converter
  ): Seq[File] =
    (libs flatMap {
  ...
  val apkbuildAggregateTaskDef = Def.task {
    Aggregate.Apkbuild(
      ...
      apkbuildDebug.value(),
      apkDebugSigningConfig.value,
      processManifest.value,
      dex.value,
      predex.value,
      collectJni.value,
      resourceShrinker.value,
      minSdkVersion.value.toInt
    )
  }
  ...
  val apkbuildTaskDef = Def.task {
    ...
    val dcp = (dependencyClasspath in Runtime).value.filterNot(
      _.data == layout.classesJar
    )
    val s = streams.value
  ...
  val zipalignTaskDef = Def.task {
    ...
    if (r.getName.contains("-unsigned")) {
      ...
      if (checkalign != 0) {
        val rv =
          Seq(z, "-f", "-p", "4", r.getAbsolutePath, aligned.getAbsolutePath).!
  ...
  val renderscriptTaskDef =
    Def.task {
      ...
      FileFunction.cached(
        ...
        bldr.compileAllRenderscriptFiles(
          Seq(layout.renderscript).asJava,
          List.empty.asJava,
          layout.rsSrc,
          layout.rsRes,
          layout.rsObj,
  ...
  val aidlTaskDef = Def.task {
    ...
    val tools =
      Option(m.getLatestBuildTool(SbtAndroidProgressIndicator(l.log), false))
    val aidl = tools map (_.getPath(PathId.AIDL)) getOrElse SdkLayout
      .aidl(s)
      .getCanonicalPath
  ...
  val processManifestTaskDef = Def.task {
    ...
    try {
      bldr(s.log).mergeManifestsForApplication(
        layout.manifest,
        a.overlays.filter(_.isFile).asJava,
      ...
      case e: Exception =>
        ...
        s.log.trace(e)
        fail(s"Failed to merge manifest files:\n    ${ms.mkString("\n    ")}")
    }
    if (noTestApk && !isLib) {
      ...
      val hasTestRunner = usesLibraries exists (_.attribute(
        ANDROID_NS,
        "name"
      ) exists (_ == TEST_RUNNER_LIB))

      def runnerLibTag(top: Elem) = {
        if (!hasTestRunner) {
          val runnerlib =
            new PrefixedAttribute(prefix, "name", TEST_RUNNER_LIB, Null)
          val usesLib = new Elem(
            null,
            USES_LIBRARY_TAG,
            runnerlib,
            TopScope,
          ...
          val u = top.copy(
            child = top.child.updated(
              top.child.indexOf(application.head),
              application.head
                .asInstanceOf[Elem]
                .copy(child = application.head.child ++ usesLib)
            )
          )
          u
        } else top
      ...
      def instrumentTag(top: Elem) = {
        if (instrument.isEmpty) {
          trunner.fold(top) { tr =>
            val target =
              new PrefixedAttribute(prefix, "targetPackage", pkg, Null)
            val label =
              new PrefixedAttribute(prefix, "label", "Test Runner", target)
            val name = new PrefixedAttribute(prefix, "name", tr, label)
            val instrumentation = new Elem(
              null,
              INSTRUMENTATION_TAG,
              name,
              TopScope,
      ...
      XML.save(
        output.getAbsolutePath,
        instrumentTag(runnerLibTag(top)),
        "utf-8",
        xmlDecl = true,
        null
      )
  ...
  val rGeneratorTaskDef = Def.task {
    ...
    withCachedRes(
      s,
      "R.java",
      normalres(layout, extrares, libs),
      genres(layout, libs)
    ) {
  ...
  def withCachedRes(
      ...
      inStamp: Seq[File],
      inHash: Seq[File]
  )(body: => Set[File]): Seq[File] = {
    var dirty = false
    if (inStamp.isEmpty && inHash.isEmpty) body.toSeq
    else {
      (FileFunction.cached(s.cacheDirectory / tag, FilesInfo.lastModified) {
        _ =>
          dirty = true
          body
      }(inStamp.toSet) ++
        FileFunction.cached(
          s.cacheDirectory / (tag + "-hash"),
          FilesInfo.hash
        ) { _ =>
          if (!dirty) body
          else Set.empty
  ...
  val proguardConfigTaskDef = Def.task {
    ...
    base ++ lines(proguardProject) ++ lines(proguardTxt) ++ aarConfig: Seq[
      String
    ]
  }
  ...
  val retrolambdaAggregateTaskDef = Def.task {
    Aggregate.Retrolambda(
      retrolambdaEnabled.value,
      Project.extract(state.value).currentUnit.classpath,
      bootClasspath.value.map(_.data),
      builder.value
    )
  }
  ...
  val aaptAggregateTaskDef = Def.task {
    Aggregate.Aapt(
      builder.value,
      apkbuildDebug.value(),
      pseudoLocalesEnabled.value,
      resConfigs.value,
      aaptAdditionalParams.value
    )
  }
  ...
  val dexAggregateTaskDef = Def.task {
    Aggregate.Dex(
      ...
      dexMulti.value,
      dexMainClassesConfig.value,
      dexMinimizeMain.value,
      dexInProcess.value,
      buildToolInfo.value,
  ...
  val predexTaskDef = Def.task {
    ...
    val shards = dexShards.value
    val legacy = dexLegacyMode.value
    val skip0 = (proguardInputs.value.proguardCache.toList ++ predexSkip.value)
      .map(_.getCanonicalFile)
      .toSet
    val skip =
      if (!predexRetrolambda.value)
        skip0 + RetrolambdaSupport.processedJar(layout.dex)
      else skip0
  ...
  val resourceShrinkerTaskDef = Def.task {
    ...
    if (jar.isDefined && doShrink && !ResourceUsageAnalyzer.TWO_PASS_AAPT) {
      ...
      if (unused > 0) {
        ...
        log.info(s"Resource Shrinker: data reduced from ${Packaging
            .sizeString(before)} to ${Packaging.sizeString(after)}, removed $pct%")
      }
      shrunkResApk
    } else {
      resApk
  ...
  val testTaskDef = Def.task {
    ...
    if (!noTestApk) {
      ...
      val trunner = instrData flatMap (_._1) getOrElse runner
      ...
      bldr.mergeManifestsForTestVariant(
        testPackage,
        minSdk,
        targetSdk,
        tpkg,
        ...
        manifestFile,
        libs.asJava,
        (placeholders: Map[String, Object]).asJava,
      ...
      Packaging.packageApk(
        apk,
        bldr,
        minSdk.toInt,
        processedManifest,
        res,
        ta.debugSigningConfig,
        layout.testAssets,
        List(dex),
  ...
  private def generateTestManifest(
      pkg: String,
      manifestOut: File,
      runner: String
  ) = {
    ...
    val pkgAttr =
      new UnprefixedAttribute("package", pkg + ".instrumentTest", vc)
    val ns = NamespaceBinding(ANDROID_PREFIX, ANDROID_NS, tns)

    val minSdk =
      new PrefixedAttribute(ANDROID_PREFIX, "minSdkVersion", "3", Null)
    val usesSdk =
      new Elem(null, "uses-sdk", minSdk, TopScope, minimizeEmpty = true)
    val runnerlib =
      new PrefixedAttribute(ANDROID_PREFIX, "name", TEST_RUNNER_LIB, Null)
    val usesLib = new Elem(
      null,
      USES_LIBRARY_TAG,
      runnerlib,
    ...
    val name = new PrefixedAttribute(ANDROID_PREFIX, "name", runner, Null)
    val instrumentation =
      new Elem(null, INSTRUMENTATION_TAG, name, TopScope, minimizeEmpty = true)
    val manifest = new Elem(
      null,
      "manifest",
      pkgAttr,
      ns,
  ...
  private def runTests(
      ...
      timeo: Int,
      all: Boolean,
      testSelection: Option[String] = None
  ) {
    ...
    def execute(d: IDevice): Unit = {
      ...
      s.log.info(
        s"Testing on ${d.getProperty(IDevice.PROP_DEVICE_MODEL)} (${d.getSerialNumber})..."
      )
      ...
      if (listener.failures.nonEmpty) {
        PluginFail(
          "Tests failed: " + listener.failures.size + "\n" +
  ...
  val cleanTaskDef = Def.task {
    ...
    def execute(d: IDevice): Unit = {
      ...
      s.log.info(s"Clearing data for $pkg on ${d
          .getProperty(IDevice.PROP_DEVICE_MODEL)} (${d.getSerialNumber})...")
      s.log.debug("Executing [%s]" format command)
      d.executeShellCommand(command, receiver)
    }
  ...
  def runTaskDef(debug: Boolean): Def.Initialize[InputTask[Unit]] =
    Def.inputTask {
      val k = sdkPath.value
      val l = projectLayout.value
      ...
      if (isLib)
        PluginFail(
          "This project is not runnable, it has set 'libraryProject := true"
        )

      val manifestXml = l.processedManifest
      val m = XML.loadFile(manifestXml)
      // if an arg is specified, try to launch that
      parsers.activityParser.parsed orElse parsers
        .findMainActivities(m)
        .headOption
        .map(activity => {
        ...
        case Some(intent) =>
          val receiver = new Commands.ShellLogging(l => s.log.info(l))
          val command =
            "am start %s -n %s" format (if (debug) "-D" else "", intent)
          def execute(d: IDevice): Unit = {
            s.log.info(
              s"Running on ${d.getProperty(IDevice.PROP_DEVICE_MODEL)} (${d.getSerialNumber})..."
            )
            s.log.debug("Executing [%s]" format command)
            d.executeShellCommand(command, receiver)
  ...
  val installTaskDef = Def.task {
    ...
    if (!l) {
      def execute(d: IDevice): Unit = {
        val cacheName =
          "install-" + URLEncoder.encode(d.getSerialNumber, "utf-8")
        FileFunction.cached(s.cacheDirectory / cacheName, FilesInfo.hash) {
          in =>
            s.log.info(
              s"Installing to ${d.getProperty(IDevice.PROP_DEVICE_MODEL)} (${d.getSerialNumber})..."
            )
  ...
  def logRate[A](log: Logger, msg: String, size: Long)(f: => A): A = {
    ...
    log.info(
      "%s %s in %.2fs. %.2f%s/s" format (msg, Packaging.sizeString(
        size
      ), secs, mrate,
  ...
  def uninstallPackage(
    ...
    val cacheName =
      "install-" + URLEncoder.encode(device.getSerialNumber, "utf-8")
    cacheDir foreach { c =>
    ...
    log.info(
      s"Uninstalling from ${device.getProperty(IDevice.PROP_DEVICE_MODEL)} (${device.getSerialNumber})..."
    )
  ...
  def loadLibraryReferences(b: File, props: Properties, prefix: String = "")(
    ...
    p.keys
      ...
      .sortWith { (a, b) => a < b }
      .flatMap { k =>
        AutoLibraryProject(b / p(k)) +: loadLibraryReferences(
          b / p(k),
          loadProperties(b / p(k)),
          k
        )
      }
      .distinct
...
//        case _: AarLibrary         => true
      ...
      case a @ AarLibrary(_) =>
        Attributed
          .blank(a.getJarFile.getCanonicalFile)
          .put(moduleID.key, a.moduleID)
      ...
      d <- l collect { case l: AarLibrary =>
        l
      }
      j <- d.getLocalJars.asScala
    } yield Attributed
      .blank(j.getCanonicalFile)
      .put(moduleID.key, d.moduleID)) ++ (for {
      d <- Seq(b / "libs", b / "lib")
      j <- (d * "*.jar").get
