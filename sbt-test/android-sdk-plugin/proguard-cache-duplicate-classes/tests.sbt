...
TaskKey[Unit]("check-dex") := {
  ...
  val hasAbstractSuite = lines exists { l =>
    l.trim.startsWith("Class descriptor") && l.trim.endsWith(
      "AbstractSuite$class;'"
    )
  }
  if (!hasAbstractSuite)
...
TaskKey[Unit]("check-cache") := {
  ...
  val suite = Stream.continually(in.getNextEntry) takeWhile (_ != null) exists {
    e => e.getName.endsWith("AbstractSuite$class.class")
  }
