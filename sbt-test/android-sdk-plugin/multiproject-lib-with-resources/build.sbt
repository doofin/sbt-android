...
// meta project
lazy val root =
  project.in(file(".")).aggregate(guidemate, geophon, guidemate_lib)

// android application project
lazy val guidemate = Project(id = "app", base = file("app"))
  .enablePlugins(AndroidApp)
  .dependsOn(guidemate_lib)
  .settings(appSettings: _*)
lazy val geophon = Project(id = "app2", base = file("app2"))
  .enablePlugins(AndroidApp)
  .dependsOn(guidemate_lib)
  .settings(appSettings: _*)

val guidemate_lib = Project(id = "lib", base = file("lib-with-resources"))
...
lazy val appSettings = List(
  ...
  packagingOptions in Android := PackagingOptions(excludes =
    Seq("META-INF/LICENSE.txt", "META-INF/NOTICE.txt")
  )
)
